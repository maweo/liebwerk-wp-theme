...
<?php if ($has_orders): ?>
    ...
    <table
        class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table p-2">
        <thead>
            <tr>
        ...
        <tbody>
            ...
            foreach ($customer_orders->orders as $customer_order) {
                ...
                <tr
                    ...
                    <?php foreach (wc_get_account_orders_columns() as $column_id => $column_name): ?>
                        <td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr($column_id); ?> py-3"
                            ...
                            <?php elseif ('order-date' === $column_id): ?>
                                <strong><time
                                        datetime="<?php echo esc_attr($order->get_date_created()->date('c')); ?>"><?php echo esc_html(wc_format_datetime($order->get_date_created())); ?></time></strong>
                            ...
                            <?php elseif ('order-actions' === $column_id): ?>
                                ...
                                if (!empty($actions)) {
                                    foreach ($actions as $key => $action) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
                                        echo '<a href="' . esc_url($action['url']) . '" class="liebwerk-account__order-button">' . esc_html($action['name']) . '</a>';
                                    }
                                }
                                ?>
                            <?php endif; ?>
...
<?php else: ?>
    <div class="liebwerk-account__no-order-info pb-5">
        <?php esc_html_e('No order has been made yet.', 'woocommerce'); ?>
        <a class="liebwerk-account__account-button"
